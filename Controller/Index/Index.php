...
class Index extends \Magento\Framework\App\Action\Action
{
        ...
        foreach ($ordercontent->orderlines->orderline as $orderline) {
            if ($orderline->productcode[0] != 'FREIGHT') {
                ...
                if (!$weightunit || $weightunit == 'g') {
                    $weightactual = (float)($weight);
                } elseif ($weightunit == 'kg') {
                    $weightactual = (float)($weight / 1000);
                } elseif ($weightunit == 'pounds') {
                    $weightactual = (float)($weight / 453.592);
                } elseif ($weightunit == 'ounces') {
                    $weightactual = (float)($weight / 28.3495);
                }
    ...
    private function _processQuote($quote, $xml, $store, $request)
    {
        ...
        foreach ($regionCollection as $region) {
            if (in_array($billing_address->division, [$region['code'], $region['name']])) {
        ...
        $addressBilling = [
            ...
            'street' => (string)$billing_address->address1.
        ...
        $regionsel_id_ship = 0;
        foreach ($regionCollection as $region) {
            if (in_array($shipping_address->division, [$region['code'], $region['name']])) {
        ...
        $addressShipping = [
            ...
            'street' => (string)$shipping_address->address1.
        ...
        $customerInstruction = @count($ordercontent->instructions) ? (string)($ordercontent->instructions) : '';
